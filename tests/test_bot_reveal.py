"""Tests for the spoiler reveal flow."""
import asyncio
import os
import pytest
...
# Set BOT_TOKEN before importing bot so the startup check doesn't raise.
...
from helpers import SPOILER_DELAY
from handlers import handle_slot
...
class TestHandleSlotSpoiler:
    ...
    def test_reply_uses_spoiler_entity(self):
        ...
        entities = msg.reply_text.call_args.kwargs["entities"]
    ...
    def test_spoiler_entity_length_matches_utf16(self):
        ...
        assert msg.reply_text.call_args.kwargs["entities"][0].length == expected

    def test_reply_is_silent(self):
        msg, _ = self._run_slot()
        assert msg.reply_text.call_args.kwargs.get("disable_notification") is True
    ...
    def test_adds_pending_reveal_to_db(self):
        ...
        assert isinstance(r.reveal_text, str) and r.reveal_text

    def test_reveal_text_contains_markdown_bold(self):
        self._run_slot()
        assert "*" in db.get_pending_reveals()[0].reveal_text

    def test_reveal_text_differs_from_spoiler_text(self):
        """Spoiler text is plain; reveal text has Markdown formatting."""
        msg, _ = self._run_slot()
    ...
    def test_schedules_reveal_job(self):
        ...
        assert ctx.application.job_queue.run_once.call_args.kwargs["when"] == SPOILER_DELAY

    def test_job_data_matches_db_row(self):
        _, ctx = self._run_slot()
        r        = db.get_pending_reveals()[0]
        job_data = ctx.application.job_queue.run_once.call_args.kwargs["data"]
        assert job_data == (r.chat_id, r.message_id, r.reveal_text, r.id)

    def test_insufficient_funds_skips_reveal(self):
        """No spoiler message or DB row when the player can't afford the spin."""
